...
CONTOUR_LAT_CORRECTION =0.4 ; 
CONTOUR_LINE_WIDTH     =3.5;
CONTOUR_COLOR   =    0x0000FF  ; 
CONTOUR_OPACITY =   0.9  ; 
MSLP_LAYER      = "mslp" ; 
...
const contourContainer = new PIXI.Container();
...
window.lastLoadedMslpRaster = null;

async function fetchAndLoadMslp(date) {
    ...
    // CORRECTION 1 : Assure la présence du "?" entre l'URL de base et les paramètres query
    let baseUrl = WCS_BASE_URL;
    if (!baseUrl.endsWith('?') && !baseUrl.includes('?')) {
        baseUrl += '?';
    }

    let mslpUrl = baseUrl;
    mslpUrl += 'SERVICE=WCS&REQUEST=GetCoverage&VERSION=2.0.1&';
    mslpUrl += 'CoverageId=' + MSLP_LAYER + '&FORMAT=image/tiff&'; 
    mslpUrl += 'SUBSETTINGCRS=http://www.opengis.net/def/crs/EPSG/0/3857&';
    mslpUrl += 'OUTPUTCRS=urn:ogc:def:crs:EPSG::3857&'; 
    mslpUrl += 'subset=http://www.opengis.net/def/axis/OGC/0/Y(' + ymin + ',' + ymax + ')&';
    mslpUrl += 'subset=http://www.opengis.net/def/axis/OGC/0/X(' + xmin + ',' + xmax + ')&';
    mslpUrl += 'SUBSET=http://www.opengis.net/def/axis/OGC/0/time("' + date + '")';
    
    // CORRECTION 2 : Si l'URL demandée est exactement identique à la dernière chargée, 
    // on évite la requête inutile. Sinon, on force le rechargement pour le Zoom/Pan/Date.
    if (window.lastMslpUrl === mslpUrl && window.lastLoadedMslpRaster) {
        return window.lastLoadedMslpRaster;
    }

    console.log("Extraction explicite du Raster MSLP depuis :", mslpUrl);

    try {
        const response = await fetch(mslpUrl);
        
        // Sécurité si GeoServer renvoie une erreur (Ex: XML d'erreur au lieu d'un TIFF)
        if (!response.ok) {
            throw new Error(`Réponse GeoServer incorrecte: ${response.status} ${response.statusText}`);
        }
        ...
        window.lastMslpUrl = mslpUrl;
        window.lastLoadedMslpRaster = georaster;
        return georaster;
    } catch (error) {
        console.error("Erreur lors du chargement explicite du WCS MSLP :", error);
        return null;
    }
}
...
async function loadGeoTIFF(date, layer_name) {		
    ...
    // CORRECTION CRITIQUE : Force PixiJS à charger immédiatement les pixels du canvas dans le GPU
    if (localTexture.baseTexture) {
        localTexture.baseTexture.update();
    } else {
        localTexture.update();
    }
...
const singlePixiOverlay = L.pixiOverlay((utils) => {
    ...
    // Recalage de la texture de fond sur la carte Leaflet
    ...
    // Initialisation des particules de vent (sans bloquer le flux de rendu)
    if (!isWindInitialized && windGrid.uData) {
        initWindParticles(renderer, globalContainer);         
        // CORRECTION 1 : Suppression du 'return;' pour permettre au reste du calque de s'afficher en même temps
    }
    
    // CORRECTION 2 : On force le rendu graphique SYSTÉMATIQUEMENT 
    // pour afficher la texture instantanément dès qu'elle arrive du réseau
...
/*const contourOverlay = L.pixiOverlay((utils) => {
    ...
    if (globalSprite.texture && globalSprite.texture.valid && window.lastLoadedGeoRaster) {        
        if (!window.contourGraphics) {
            ...
            contourContainer.addChild(window.contourGraphics);
        }        
        window.contourGraphics.clear();
        window.contourGraphics.visible = true;    
            
            const georaster = window.lastLoadedGeoRaster;
const width = georaster.width;
const height = georaster.height;

// 1. Allocation d'un tableau 1D de la taille exacte requise par D3
const flatRasterData = new Float32Array(width * height);
const rasterRows = georaster.values[0]; // Première bande de données

// 2. La boucle de reconstruction pour passer de la 2D à la 1D linéaire
let index = 0;
for (let y = 0; y < height; y++) {
    const row = rasterRows[y];
    for (let x = 0; x < width; x++) {
        flatRasterData[index] = row[x];
        index++;
    }
}
...
// 3. Génération des seuils d3
const thresholds = d3.range(minVal, maxVal, (maxVal - minVal) / 10);


// 4. Initialisation du générateur avec la taille exacte de la bande
const contourGenerator = d3.contours()
    ...
    .thresholds(thresholds);
        ...
        contourFeatures.forEach(feature => {
            ...
            window.contourGraphics.lineStyle(CONTOUR_LINE_WIDTH, CONTOUR_COLOR, CONTOUR_OPACITY);
            coordinates.forEach(polygon => {
                ...
                polygon.forEach(ring => {
                    ...
                    // CORRECTION 1 : Extraction correcte de X (index 0) et Y (index 1) du pixel
                    const getScreenPoint = (pixelCoord) => {
				
                        const x_3857 = xmin_3857 + (pixelCoord[0] * resX);
                        const y_3857 = ymax_3857 - (pixelCoord[1] * resY);           
                        const point3857 = L.point(x_3857, y_3857);
                        const latLngDegres = map.options.crs.unproject(point3857);                            
                        const latLngCorrige = L.latLng(latLngDegres.lat - CONTOUR_LAT_CORRECTION , latLngDegres.lng);
                        return project(latLngCorrige);
                    };                    
                    const startPt = getScreenPoint(ring[0]);
    ...
    // Force PixiJS à afficher le conteneur mis à jour
    renderer.render(contourContainer);
}, contourContainer);
//contourOverlay.addTo(map) ;
//}*/


// Initialisation du conteneur spécifique s'il n'existe pas
//const contourContainer = new PIXI.Container();
//globalContainer.addChild(contourContainer); // Placé au-dessus du fond global

const contourOverlay = L.pixiOverlay((utils) => {
    const renderer = utils.getRenderer();
    const project  = utils.latLngToLayerPoint;    
    
    if (window.lastLoadedMslpRaster) {        
        if (!window.contourGraphics) {
            window.contourGraphics = new PIXI.Graphics();
            contourContainer.addChild(window.contourGraphics);
        }        
        ...
        const georaster = window.lastLoadedMslpRaster;
        const width = georaster.width;
        const height = georaster.height;        
        // 1. Reconstruction du Float32Array
        const flatRasterData = new Float32Array(width * height);
        const rasterRows = georaster.values[0]; 
        ...
        // 2. Calcul des seuils et des features
        const minVal = Math.min(...flatRasterData);
        const maxVal = Math.max(...flatRasterData);
        const thresholds = d3.range(minVal, maxVal, 5);
        
        const contourGenerator = d3.contours().size([width, height]).thresholds(thresholds);
        const contourFeatures = contourGenerator(flatRasterData);
        ...
        // =================================================================
        // CORRECTION MAJEURE : On calcule le coin supérieur gauche (Top-Left)
        // et le coin inférieur droit (Bottom-Right) du Raster EN UNE SEULE FOIS.
        // On n'appellera PLUS JAMAIS 'project()' dans la boucle des points !
        // =================================================================
        const ptTopLeft_3857 = L.point(xmin_3857, ymax_3857);
        const latLngTopLeft = map.options.crs.unproject(ptTopLeft_3857);
        // On applique votre décalage météo de -0.2 ici une seule fois
        const screenTopLeft = project(L.latLng(latLngTopLeft.lat - 0.2, latLngTopLeft.lng));

        const ptBottomRight_3857 = L.point(xmin_3857 + (width * resX), ymax_3857 - (height * resY));
        const latLngBottomRight = map.options.crs.unproject(ptBottomRight_3857);
        const screenBottomRight = project(L.latLng(latLngBottomRight.lat - 0.2, latLngBottomRight.lng));

        // Calcul de la taille et de l'échelle PixiJS pour ce raster
        const rasterScreenWidth = screenBottomRight.x - screenTopLeft.x;
        const rasterScreenHeight = screenBottomRight.y - screenTopLeft.y;
        const scaleX = rasterScreenWidth / width;
        const scaleY = rasterScreenHeight / height;

        // Fonction mathématique ultra-rapide sans aucun appel à Leaflet
        const getFastScreenPoint = (pixelCoord) => {
            return {
                x: screenTopLeft.x + (pixelCoord[0] * scaleX),
                y: screenTopLeft.y + (pixelCoord[1] * scaleY)
            };
        };

        // 3. Tracé des contours (Devenu ultra léger pour la mémoire)
        contourFeatures.forEach(feature => {
            const coordinates = feature ? feature.coordinates : null;
            if (!coordinates || coordinates.length === 0) return;                
            
            window.contourGraphics.lineStyle(2.5, 0xFFFFFF, 0.8);            
            coordinates.forEach(polygon => {
                polygon.forEach(ring => {
                    if (!ring || ring.length === 0) return;                   
                    
                    // Utilisation de la version optimisée sans saturation de pile
                    const startPt = getFastScreenPoint(ring[0]);
                    window.contourGraphics.moveTo(startPt.x, startPt.y);

                    for (let i = 1; i < ring.length; i++) {
                        const currentPt = getFastScreenPoint(ring[i]);
                        window.contourGraphics.lineTo(currentPt.x, currentPt.y);
                    }
                });
            });
...
/*document.addEventListener("DOMContentLoaded", () => {
    ...
    if (mslpCheckbox) {
        mslpCheckbox.addEventListener('change', () => {
            console.log("Add  MSLP contours :", mslpCheckbox.checked);            
            if (mslpCheckbox.checked) {
                // Évite d'ajouter plusieurs fois la couche si elle y est déjà
                if (!map.hasLayer(contourOverlay)) {
			       var date =window.timeSteps[currentIndex] ; 
                    showMslp (date) ;
                    //contourOverlay.addTo(map);
                }            
                // CORRECTION 2 & 4 : On force le redessin des DEUX overlays Pixi
                // Cela force le moteur à lier le shader de fond et les nouvelles lignes
                //contourOverlay.redraw();
                /*if (typeof singlePixiOverlay !== 'undefined') {
                    singlePixiOverlay.redraw(); 
                }
            } else {
                // CORRECTION 3 : Nettoyage ciblé de l'overlay de contours
                if (window.contourGraphics) {
                    window.contourGraphics.clear();
                ...
                // On rafraîchit le fond pour effacer proprement les résidus
                /*if (typeof singlePixiOverlay !== 'undefined') {
                    singlePixiOverlay.redraw();
                }
            }
...
document.addEventListener("DOMContentLoaded", () => {
    const mslpCheckbox = document.getElementById('overlay-mslp');    
    if (mslpCheckbox) {
        mslpCheckbox.addEventListener('change', async () => {
            if (mslpCheckbox.checked) {
                // 1. On charge le raster de manière explicite
                const current_time = window.timeSteps[window.currentIndex] ;
                const rasterPret = await fetchAndLoadMslp(current_time);
                
                if (rasterPret) {
                    // 2. On ajoute l'overlay s'il n'y est pas
                    if (!map.hasLayer(contourOverlay)) {
                        contourOverlay.addTo(map); // Méthode Leaflet standard
                    }
                    
                    // CORRECTION : Un seul redraw suffit amplement. 
                    // Supprimez singlePixiOverlay.redraw() d'ici pour briser la boucle infinie.
                    contourOverlay.redraw();
                }
            } else {
                // Nettoyage au décochage
                if (window.contourGraphics) {
                    window.contourGraphics.clear();
                }
                if (map.hasLayer(contourOverlay)) {
